...
10倍股基准池位于 `tools/tenx_universe.txt`，共 2,345 个唯一股票代码，完整对应 2026-08-08 核对版的“全量股票池”。每日任务会处理全部代码，而不是只取市值靠前的精选样本；公开接口暂时取数失败的股票仍保留在页面并显示数据不足。仓库仅保存代码清单及公开数据计算结果，不保存账号、会话、原始附件、付费评分或报告原文。
