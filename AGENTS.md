...
用户所说的「飄网站」固定指本仓库发布的站点：`https://langlang2019613.github.io/P-investing/`。当用户要求保存到「飄网站」时，按下述记录与发布流程执行。
...
1. 把本次分析/总结整理成完整 Markdown，保存为：
   ...
   - `interviews` 访谈（企业家、管理层、学者与政策人物的长篇访谈纪要）
   - `learning`   学习（课程笔记、方法论、框架、技能积累）
   - `books`      书刊笔记（书籍与杂志的阅读笔记）
   - `history`    经济历史

   注意：网站内容不出现「5+2」提法（统一用 18维度），不出现视频平台
   （B站/YouTube等）及其博主/栏目名称，不出现「视频总结」等字样，
   也不出现「微信、一口气、吊打、赢麻了、震撼、爆出」等来源与标题噪音词
   ——标题一律用中性专业表述。
